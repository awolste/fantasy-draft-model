...
*Why 2024 and not 2025:* the opponent model learns how each manager drafts **relative to that year's ADP**, so it needs historical ADP, and so does the backtest. Fantasy Football Calculator — the only free source with deep historical ADP — has 2018–2024 and 2026 but **no 2025 data at all**, verified across every scoring type. ESPN was checked as an alternative and does not fill the gap either (below). The 2025 season therefore cannot contribute to ADP-relative fitting and cannot serve as the holdout. Its draft picks remain available from ESPN and are still usable for anything not requiring ADP.

*ESPN ADP — usable for 2023, 2024, and live 2026 only.* ESPN exposes `ownership.averageDraftPosition` via the `kona_player_info` view. Verified season by season: 2023, 2024, and 2026 return genuine values; **every other season returns sentinel padding**, not data — 2018 and 2025 give `170.0` for every player including Chase and Jefferson, 2019 gives `0`, and 2020–2022 give ~`169.9`. The `170.0` sentinel is ESPN's "undrafted" default and coincidentally equals the league's pre-2023 draft size, so it reads as plausible at a glance. Draft-rank sorting is also unavailable before 2023.

ESPN ADP is therefore not a substitute for FFC, but it is worth ingesting for **2023, 2024, and 2026 as a secondary signal in Stage 3**: leaguemates draft on ESPN and see ESPN's ADP, so it is a closer proxy for the anchor they actually respond to. Disagreement between ESPN and FFC ADP is itself informative about how much the anchor matters.
